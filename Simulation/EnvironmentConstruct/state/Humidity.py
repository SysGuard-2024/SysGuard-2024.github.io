...
class Humidity(): # -2 -1 0 1 2
    ...
    def ext_action_decrease(self, env):
        ...
        if self._enable_decrease():
            ...
            if response.status_code == 200:
                print(self.room + " humidity_down 成功")
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["humidity_down", "Event", self.room, 'Humidity', Time, self.room + '.Humidity.state: ' + value, 'Environment Change']
                globalFrame.afterDeal(env, "humidity_down", self.room, self.room + '.Humidity.state: ' + value, self.space)
            else:
                print(self.room + " humidity_down 失败")
        else:
            self.lock.release()
    ...
    def ext_action_increase(self, env):
        ...
        if self._enable_increase():
            ...
            self.lock.release()
            if response.status_code == 200:
                print(self.room + " humidity_up 成功")
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["humidity_up", "Event", self.room, 'Humidity', Time, self.room + '.Humidity.state: ' + value, 'Environment Change']
                globalFrame.afterDeal(env, "humidity_up", self.room, self.room + '.Humidity.state: ' + value, self.space)
            else:
                print(self.room + " humidity_up 失败")
        else:
            self.lock.release()
