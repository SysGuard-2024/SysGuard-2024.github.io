...
class AirQuality(): # -2 -1 0 1
    ...
    def ext_action_decrease(self, env):
        ...
        if self._enable_decrease():
            ...
            if response.status_code == 200:
                print(self.room + " air_quality_down 成功")
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["air_quality_down", "Event", self.room, 'AirQuality', Time, self.room + '.AirQuality.state: ' + value, 'Environment Change']
                globalFrame.afterDeal(env, "air_quality_down", self.room, self.room + '.AirQuality.state: ' + value, self.space)
            else:
                print(self.room + " air_quality_down 失败")
        else:
            self.lock.release()
    ...
    def ext_action_increase(self, env):
        ...
        if self._enable_increase():
            ...
            self.lock.release()
            if response.status_code == 200:
                print(self.room + " air_quality_up 成功")
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["air_quality_up", "Event", self.room, 'AirQuality', Time, self.room + '.AirQuality.state: ' + value, 'Environment Change']
                globalFrame.afterDeal(env, "air_quality_up", self.room, self.room + '.AirQuality.state: ' + value, self.space)
            else:
                print(self.room + " air_quality_up 失败")
        else:
            self.lock.release()
