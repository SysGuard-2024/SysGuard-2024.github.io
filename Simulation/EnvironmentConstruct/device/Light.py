...
class Light():
    ...
    def action_on(self, env, source): 
        ...
        if self._enable_on():
            ...
            if response.status_code == 200:
                ...
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["light_on", "Action", self.room_name, self.device_name, Time, self.room_name + '.' + self.device_name + '.state: 1', source]
                globalFrame.afterDeal(env, "light_on", self.room_name, self.room_name + '.' + self.device_name + '.state: 1', self.space)
                self.flag_on.set()
                self.flag_off.clear()
            else:
    ...
    def effect_on(self, env):
        ...
        temp_state = env["space_dict"][self.room_name]["env_state"]["Brightness"]
        while not self.running.is_set() and not self.running.is_set():
            if self.flag_on.is_set():
                interval = 1 - int(temp_state.ap_value())
                self.pre_value = interval
                temp_state.set_value(interval)
    ...
    def effect_off(self, env):
        ...
        while not self.running.is_set():
            if self.flag_off.is_set():
                temp_state.set_value(-1*self.pre_value)
                energy_state.set_value(-0.1)
    ...
    def action_off(self, env, source):        
        ...
        if self._enable_off():
            ...
            if response.status_code == 200:
                ...
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["light_off", "Action", self.room_name, self.device_name, Time, self.room_name + '.' + self.device_name + '.state: 0', source]
                globalFrame.afterDeal(env, "light_off", self.room_name, self.room_name + '.' + self.device_name + '.state: 0', self.space)
                self.flag_off.set()
                self.flag_on.clear()
            else:
