    ...
    def action_on(self, env, source): 
        ...
        if self._enable_on():
            ...
            if response.status_code == 200:
                ...
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["water_dispenser_on", "Action", self.room_name, self.device_name, Time, self.room_name + '.' + self.device_name + '.state: 1', source]
                globalFrame.afterDeal(env, "water_dispenser_on", self.room_name, self.room_name + '.' + self.device_name + '.state: 1', self.space)
                self.flag_on.set()
                self.flag_off.clear()
            else:
    ...
    def action_off(self, env, source): 
        ...
        if self._enable_off():
            ...
            if response.status_code == 200:
                ...
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["water_dispenser_off", "Action", self.room_name, self.device_name, Time, self.room_name + '.' + self.device_name + '.state: 0', source]
                globalFrame.afterDeal(env, "water_dispenser_off", self.room_name, self.room_name + '.' + self.device_name + '.state: 0', self.space)
                self.flag_off.set()
                self.flag_on.clear()
            else:
