...
class Noise(): # -1 0 1 2
    ...
    def ext_action_decrease(self, env):
        ...
        if self._enable_decrease():
            ...
            if response.status_code == 200:
                print(self.room + " noise_down 成功")
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["noise_down", "Event", self.room, 'Noise', Time, self.room + '.Noise.state: ' + value, 'Environment Change']
                globalFrame.afterDeal(env, "noise_down", self.room, self.room + '.Noise.state: ' + value, self.space)
            else:
                print(self.room + " noise_down 失败")
        else:
            self.lock.release()
    ...
    def ext_action_increase(self, env):
        ...
        if self._enable_increase():
            ...
            self.lock.release()
            if response.status_code == 200:
                print(self.room + " noise_up 成功")
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["noise_up", "Event", self.room, 'Noise', Time, self.room + '.Noise.state: ' + value, 'Environment Change']
                globalFrame.afterDeal(env, "noise_up", self.room, self.room + '.Noise.state: ' + value, self.space)
            else:
                print(self.room + " noise_up 失败")
        else:
            self.lock.release()
