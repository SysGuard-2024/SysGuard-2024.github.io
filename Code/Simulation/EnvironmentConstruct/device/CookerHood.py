...
class CookerHood():
    ...
    def action_on(self, env, source): 
        ...
        if self._enable_on():
            ...
            if response.status_code == 200:
                ...
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["cooker_hood_on", "Action", self.room_name, self.device_name, Time, self.room_name + '.' + self.device_name + '.state: 1', source]
                globalFrame.afterDeal(env, "cooker_hood_on", self.room_name, self.room_name + '.' + self.device_name + '.state: 1', self.space)
                time.sleep(0.05)
                self.flag_on.set()
                self.flag_off.clear()
            else:
    ...
    def effect_on(self, env):
        ...
        air_state = env["space_dict"][self.room_name]["env_state"]["AirQuality"]
        c02_state = env["space_dict"][self.room_name]["env_state"]["Co2Level"]
        noise_state = env["space_dict"][self.room_name]["env_state"]["Noise"]

        while not self.running.is_set():
            if self.flag_on.is_set():
                energy_state.set_value(0.1)
                noise_state.set_value(0.5)  
                while self.flag_on.is_set() and not self.running.is_set():
                    interval = 1 - int(air_state.ap_value())
                    air_state.set_value(interval/165)
                    
                    interval = -1 - int(air_state.ap_value())
                    c02_state.set_value(interval/240)
                    time.sleep(0.08)
            time.sleep(0.08)
    ...
    def action_off(self, env, source): 
        ...
        if self._enable_off():
            ...
            if response.status_code == 200:
                ...
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["cooker_hood_off", "Action", self.room_name, self.device_name, Time, self.room_name + '.' + self.device_name + '.state: 0', source]
                globalFrame.afterDeal(env, "cooker_hood_off", self.room_name, self.room_name + '.' + self.device_name + '.state: 0', self.space)
                time.sleep(0.05)
                self.flag_off.set()
                self.flag_on.clear()
            else:
