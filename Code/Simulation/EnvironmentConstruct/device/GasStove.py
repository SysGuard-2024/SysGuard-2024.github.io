    ...
    def action_on(self, env, source): 
        ...
        if self._enable_on():
            ...
            if response.status_code == 200:
                ...
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["gas_stove_on", "Action", self.room_name, self.device_name, Time, self.room_name + '.' + self.device_name + '.state: 1', source]
                globalFrame.afterDeal(env, "gas_stove_on", self.room_name, self.room_name + '.' + self.device_name + '.state: 1', self.space)
                time.sleep(0.03)
                self.flag_on.set()
                self.flag_off.clear()
            else:
        ...
        while not self.running.is_set():
            if self.flag_on.is_set():
                ...
                while self.flag_on.is_set() and not self.running.is_set():
                    interval = -2 - int(air_state.ap_value())
                    air_state.set_value(interval/360)  
                    
                    interval = 1 - int(c02_state.ap_value())
                    c02_state.set_value(interval/450)  
                    time.sleep(0.08)        
            time.sleep(0.08)
    ...
    def action_off(self, env, source): 
        ...
        if self._enable_off():
            ...
            if response.status_code == 200:
                ...
                Time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(globalFrame.start_time))
                with self.df_lock:
                    self.df.loc[len(self.df)] = ["gas_stove_off", "Action", self.room_name, self.device_name, Time, self.room_name + '.' + self.device_name + '.state: 0', source]
                globalFrame.afterDeal(env, "gas_stove_off", self.room_name, self.room_name + '.' + self.device_name + '.state: 0', self.space)
                time.sleep(0.03)
                self.flag_off.set()
                self.flag_on.clear()
            else:
